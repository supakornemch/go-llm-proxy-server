...
if [ -z "$VK_ID" ]; then
    ...
    VK_OUT=$(./llm-proxy vkey add --db-type "$DB_TYPE" --dsn "$DB_DSN" --name "Azure-Test-$TS" --key "$VKEY")
...
run_test_azure() {
    ...
    if [[ -n "$AZURE_FOUNDRY_API_KEY" && -n "$AZURE_FOUNDRY_URL" ]]; then
        ...
        TS=$(date +%s%N)
        CONN_OUT=$(./llm-proxy connection add --db-type "$DB_TYPE" --dsn "$DB_DSN" --provider "azure" --name "Azure-Conn-$TS" --endpoint "$AZURE_FOUNDRY_URL" --api-key "$AZURE_FOUNDRY_API_KEY")
        AZURE_ID=$(echo "$CONN_OUT" | grep -oE "[a-f0-9-]{36}" | tail -1)
        ...
        MODEL_OUT=$(./llm-proxy model add --db-type "$DB_TYPE" --dsn "$DB_DSN" --conn-id "$AZURE_ID" --name "azure-gpt" --remote "gpt-oss-120b")
