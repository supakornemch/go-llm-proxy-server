...
if [ -z "$VK_ID" ]; then
    ...
    VK_ID=$(./llm-proxy vkey add --db-type "$DB_TYPE" --dsn "$DB_DSN" --name "Google-Test-$TS" --key "$VKEY" | grep -oE "[a-f0-9-]{36}" | tail -1)
...
run_test_google() {
    ...
    if [[ -n "$GOOGLE_VERTEX_API_KEY" && -n "$GOOGLE_GEMINI_ENDPOINT" ]]; then
        ...
        TS=$(date +%s)
        GOOGLE_ID=$(./llm-proxy connection add --db-type "$DB_TYPE" --dsn "$DB_DSN" --provider "google" --name "Google-Conn-$TS" --endpoint "$GOOGLE_GEMINI_ENDPOINT" --api-key "$GOOGLE_VERTEX_API_KEY" | grep -oE "[a-f0-9-]{36}" | tail -1)
